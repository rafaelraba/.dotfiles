...
- `modules/workspace_layout_restore.lua`: polls the focused AeroSpace workspace and persists saved Hammerspoon layouts; auto-restore is disabled by default.
...
- **AeroSpace**: workspace switching, move-node-to-workspace, and reload. No app-to-workspace rules or workspace-to-monitor force assignment are currently configured.
...
- Auto-restore on workspace switch is disabled by default; use the explicit layout shortcuts when a workspace needs its layout applied.
...
Do not add AeroSpace `exec-on-workspace-change` hooks or shell scripts that call `hs -c`; those bridge patterns can wedge AeroSpace IPC.
