...
workspace="${1:?Usage: move-focused-window-to-workspace.sh <workspace>}"
...
if not oldFrame.x or not sourceFrame.x or sourceFrame.w <= 0 or sourceFrame.h <= 0 then
  ...
  return 'Moved focused window to workspace and contained it on target monitor'
end
...
return 'Moved focused window to workspace and scaled it on target monitor'
"
